...
vector<float> gen_avg, g1_avg, g2_avg, g3_avg, g4_avg, gen_min, gen_max;
vector<float> g1, g2, g3, g4, all_luck, gen;
std::random_device rd;
boost::mt19937 genRand(rd());
boost::normal_distribution cd{0.0, 0.780304146072379};

float uniformRand(float m, float f) {
  float diff = abs(m - f);
...
void sortIt(vector<float> &arr) {
  sort(execution::par_unseq, arr.begin(), arr.end(), greater<float>());
}

void shuffleIt(vector<float> &arr) { shuffle(arr.begin(), arr.end(), rng); }

float avg(const vector<float> &arr) {
  return reduce(execution::par_unseq, arr.begin(), arr.end(), 0.0) / arr.size();
}

float sum(const vector<float> &arr) {
  return reduce(execution::par_unseq, arr.begin(), arr.end(), 0.0);
}

float max(const vector<float> &arr) {
  return *max_element(execution::par_unseq, arr.begin(), arr.end());
}

float min(const vector<float> &arr) {
  return *min_element(execution::par_unseq, arr.begin(), arr.end());
}

void printArr(const vector<float> &arr, const string &name) {
...
void printArrAvg(const vector<float> &arr, const string *name) {
  cout << name << " avg: " << avg(arr) << endl;
}

void printArrMax(const vector<float> &arr, const string *name) {
  cout << name << " max: " << max(arr) << endl;
}

void printArrMin(const vector<float> &arr, const string *name) {
  cout << name << " min: " << min(arr) << endl;
}

void printGroups(vector<float> &arr) {
  int index = 0;
  for (int j = 1; j <= 4; j++) {
    int newIndex = round(((float)j / 4) * arr.size());
    if (newIndex % 2 != 0) {
      --newIndex;
    }
...
void birth(vector<float> &male, vector<float> &female) {
  cout << "\n***** Generation " << genNum + 1 << " [Year: " << (genNum + 1) * 25
       << "] *****" << endl;
  int len_male = male.size();
  for (int i = len_male - 1; 0 <= i; i--) {
  ...
  for (int j = 0; j < 4; j++) {
    for (int i = 0;
         ...
         i = i + 2) {
      float pair_kid = (gen[i] + gen[i + 1]) / 2;
      float luck = uniformRand(gen[i], gen[i + 1]) + cd(genRand);
      // all_luck.push_back(luck);
      float kid = pair_kid + luck;
